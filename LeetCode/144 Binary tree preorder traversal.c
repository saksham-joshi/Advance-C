...
static void basePreOrder ( struct TreeNode* __node , VectorPtr_t __vector )
{
    if ( __node )
    {
        vectorPushBack ( __vector , __node->val );
        basePreOrder   ( __node->left , __vector );
        basePreOrder   ( __node->right , __vector );
    }
} 
inline static int* preorderTraversal(struct TreeNode* __root, int* __return_size)
{
    Vector vec = vectorInit ( 4 );

    basePreOrder ( __root , &vec );
