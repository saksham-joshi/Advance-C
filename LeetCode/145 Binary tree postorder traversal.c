...
static void basePostOrder ( struct TreeNode* __node , VectorPtr_t __vector )
{
    if ( __node )
    {
        basePostOrder   ( __node->left , __vector );
        basePostOrder   ( __node->right , __vector );
        vectorPushBack ( __vector , __node->val );
    }
} 

inline static int* postorderTraversal(struct TreeNode* __root, int* __return_size)
{
    Vector vec = vectorInit ( 8 );

    basePostOrder ( __root , &vec );
